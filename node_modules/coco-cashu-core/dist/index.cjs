let _cashu_cashu_ts = require("@cashu/cashu-ts");
...
function deserializeOutput(serialized) {
	return new _cashu_cashu_ts.OutputData({
...
function computeYHexForSecrets(secrets) {
	...
	return secrets.map((secret) => (0, _cashu_cashu_ts.hashToCurve)(encoder.encode(secret)).toHex(true));
}
/**
...
var KeyRingService = class {
	...
	async generateNewKeyPair(options) {
		...
		const hdKey = _scure_bip32.HDKey.fromMasterSeed(seed);
	...
	async signProof(proof, publicKey) {
		...
		const signature = _noble_curves_secp256k1_js.schnorr.sign((0, _noble_hashes_sha2_js.sha256)(message), keyPair.secretKey);
		const signedProof = {
			...proof,
			witness: JSON.stringify({ signatures: [(0, _noble_curves_utils_js.bytesToHex)(signature)] })
		};
	...
	*/
	getPublicKeyHex(secretKey) {
		return "02" + (0, _noble_curves_utils_js.bytesToHex)(_noble_curves_secp256k1_js.schnorr.getPublicKey(secretKey));
	}
};
...
var WalletService = class {
	...
	*/
	...
	async buildWallet(mintUrl) {
		...
		const seed = await this.seedService.getSeed();
		const wallet = new _cashu_cashu_ts.Wallet(new _cashu_cashu_ts.Mint(mintUrl, { customRequest: this.requestProvider.getRequestFn(mintUrl) }), {
			unit: DEFAULT_UNIT,
...
var ProofService = class {
	...
	*/
	...
	async createOutputsAndIncrementCounters(mintUrl, amount, options) {
		...
		if (keepAmount > 0) {
			data.keep = _cashu_cashu_ts.OutputData.createDeterministicData(keepAmount, seed, currentCounter.counter, keys);
	...
	*/
	...
	async createBlankOutputs(amount, mintUrl) {
		...
		const outputData = Array(outputNumber).fill(0).map((_, index) => {
			return _cashu_cashu_ts.OutputData.createSingleDeterministicData(0, seed, currentCounter.counter + index, keys.id);
		});
...
var WalletRestoreService = class {
	...
	async sweepKeyset(mintUrl, keysetId, bip39seed) {
		...
		const { wallet } = await this.walletService.getWalletWithActiveKeysetId(mintUrl);
		const sweepWallet = new _cashu_cashu_ts.Wallet(new _cashu_cashu_ts.Mint(mintUrl, { customRequest: this.requestProvider.getRequestFn(mintUrl) }), { bip39seed });
...
var TransactionService = class {
	...
	async receive(token) {
		let mint;
		try {
			mint = typeof token === "string" ? (0, _cashu_cashu_ts.getTokenMetadata)(token).mint : token.mint;
		} catch (err) {
		...
		try {
			...
			let proofs = typeof token === "string" ? (0, _cashu_cashu_ts.getDecodedToken)(token, keysetIds).proofs : token.proofs;
...
var PaymentRequestService = class {
	...
	*/
	...
	async readPaymentRequest(paymentRequest) {
		...
		const decodedPaymentRequest = _cashu_cashu_ts.PaymentRequest.fromEncodedRequest(paymentRequest);
	...
	getPaymentRequestTransport(pr) {
		...
		const httpTransport = pr.transport.find((t) => t.type === _cashu_cashu_ts.PaymentRequestTransportType.POST);
...
var MintAdapter = class {
	...
	getCashuMint(mintUrl) {
		if (!this.cashuMints[mintUrl]) {
			...
			this.cashuMints[mintUrl] = new _cashu_cashu_ts.Mint(mintUrl, { customRequest: requestFn });
		}
...
var WalletApi = class {
	...
	*/
	...
	async decodeToken(tokenString) {
		const metadata = (0, _cashu_cashu_ts.getTokenMetadata)(tokenString);
		return (await this.walletService.getWallet(metadata.mint)).decodeToken(tokenString);
	}
	encodeToken(token) {
		return (0, _cashu_cashu_ts.getEncodedToken)(token);
	}
};
...
exports.ExtensionRegistrationError = ExtensionRegistrationError;
...
exports.MeltOperationService = MeltOperationService;
...
exports.MemoryMeltOperationRepository = MemoryMeltOperationRepository;
...
exports.MemorySendOperationRepository = MemorySendOperationRepository;
...
exports.OperationInProgressError = OperationInProgressError;
...
exports.SendOperationService = SendOperationService;
...
exports.createSendOperation = createSendOperation;
Object.defineProperty(exports, 'getDecodedToken', {
  enumerable: true,
  get: function () {
    return _cashu_cashu_ts.getDecodedToken;
  }
});
Object.defineProperty(exports, 'getEncodedToken', {
  enumerable: true,
  get: function () {
    return _cashu_cashu_ts.getEncodedToken;
  }
});
exports.getKeepProofSecrets = getKeepProofSecrets;
exports.getSendProofSecrets = getSendProofSecrets;
exports.hasPreparedData = hasPreparedData;
exports.initializeCoco = initializeCoco;
exports.isExecutingOperation = isExecutingOperation;
exports.isFinalizedOperation = isFinalizedOperation;
